...
namespace Easy3D
{
	...
	float AnimationCurve::GetValueFromTime(float aTime)
	{
		...
		if (posInArray + 1 < myKeyFrames.Size())
		{
			...
			float timeInKey = myStartTime + timeInAnim - myKeyFrames[posInArray].myTime;

			float keyTime = myKeyFrames[posInArray + 1].myTime - myKeyFrames[posInArray].myTime;

			float alpha = timeInKey;// / keyTime;
